        ...
        {capture name="tabs_title" assign="tabs_title"}
          ...
          {foreach from=$items item="filter" name="filters"}
            ...
            {/if}
              ...
              <li id="{$id}" class="cm-js ty-tabs__item et-filter-tab {if $active_tab || $filter.field_type != "Addons\CpCustomPrice\CpCustomPriceFilterField::FILTER_FIELD_ID"|enum}active{/if}{if $tab.hidden == "Y" || $filter.field_type == "Addons\CpCustomPrice\CpCustomPriceFilterField::FILTER_FIELD_ID"|enum} hidden{/if}{if $tab.js} cm-js{elseif $tab.ajax} cm-js cm-ajax{if $tab.ajax_onclick} cm-ajax-onclick{/if}{/if}{if $tab.properties} extra-tab{/if}" data-et-id="{$filter.filter_id}">
          ...
          <div class="et-hf-tabs__content">
            {foreach from=$items item="filter" name="filters"}
              ...
              {/if}
                ...
                <div id="{$id}" class="{$id} et-tab-content {if !$active_tab || $filter.field_type == "Addons\CpCustomPrice\CpCustomPriceFilterField::FILTER_FIELD_ID"|enum}hidden{/if}">
