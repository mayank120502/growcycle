...
<div id="content_{$filter_uid}" class="cm-product-filters-checkbox-container ty-price-slider {if $collapse}hidden{/if} {$extra_class}">
	...
	<div class="et-range-slider__price-wrapper">
		<p class="ty-price-slider__inputs">
			...
			<span class="ty-price-slider__filter-prefix">{$filter.prefix nofilter}</span>
				<input type="text" class="ty-price-slider__input-text" id="slider_{$filter_uid}_left" name="left_{$filter_uid}" value="{$left}"{if $disable_slider} disabled="disabled"{/if} data-ca-previous-value="{$left}" />
				{$filter.suffix nofilter}
			</bdi>
			&nbsp;–&nbsp;
		  <bdi class="ty-price-slider__bidi-container">
			<span class="ty-price-slider__filter-prefix">{$filter.prefix nofilter}</span>
				<input type="text" class="ty-price-slider__input-text" id="slider_{$filter_uid}_right" name="right_{$filter_uid}" value="{$right}"{if $disable_slider} disabled="disabled"{/if} data-ca-previous-value="{$right}" />
			{$filter.suffix nofilter}
			</bdi>
		</p>
	</div>
	...
	{if $filter.field_type == "ProductFilterProductFieldTypes::PRICE"|enum && $is_filter_cp_custom_price}
		<div class="cp-custom-price-filter-checkbox">
			{include file="blocks/product_filters/components/product_filter_variants.tpl" filter_uid=$filter_uid_cp_custom_price
			filter=$filter_cp_custom_price collapse=false}
		</div>
	{/if}
</div>
