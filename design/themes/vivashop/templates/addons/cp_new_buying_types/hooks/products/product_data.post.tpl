        ...
        {foreach from=fn_cp_get_additional_product_buying_types(true) key=type item=but_text name=product_buying_types}
            {if in_array($type, $buying_types)}
                ...
                {if $type === $contact_vendor_type}
                    ...
                    {$icon = 'et-icon-mail'}
                    {$href = $user_data.fields.60|fn_url}
                ...
                {/if}
{if $contact_vendor_type =='C'}
  
{$fm_user_id = fn_get_company_root_admin_user_id($product.company_id)}
{if $fm_user_id}
	{$fm_user_info = fn_get_user_info($fm_user_id)}
	{if $fm_user_info}
		{$company_redirect_url_typeform = $fm_user_info.fields.60}
		{if $company_redirect_url_typeform != ''}
			{$href = $fm_user_info.fields.60}
			{$dialog_class = 'fm_not_popup'}
			{$class='ty-btn text-button et-btn-icon-text et-add-to-cart cp-additional-buying-types et-in-stock'}
		{/if}	
	{/if}	
{/if}
