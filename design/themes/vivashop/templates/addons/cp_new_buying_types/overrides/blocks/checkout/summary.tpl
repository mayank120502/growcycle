            ...
            <tr>
                <th class="ty-checkout-summary__total" colspan="2" data-ct-checkout-summary="order-total">
                    <div>
                        ...
                        <span class="ty-checkout-summary__total-sum">
                            {* overrided *}
                            {if $smarty.request.start_order}
                                <span class="cp-to-be-negotiated-text">{__('cp_new_buying_types.to_be_negotiated')}</span>
                            {else}
                                {include file="common/price.tpl" value=$_total|default:$cart.total class="ty-price ty-strong"}
                            {/if}
                            {* overrided *}
                        </span>
                    </div>
