                ...
                <a href="{"checkout.cart"|fn_url}">
                    ...
                    <span class="clearfix">
                        <span class="ty-float-left et-top-cart-icon">
                            ...
                            <span class="et-cart-content {if !$cart.amount}et-cart-empty{/if}">{strip}
                                {if $cart.amount}
                                    {$cart.amount}
                                {else}
                                    0
                    ...
                    <div class="cm-cart-content {if $block.properties.products_links_type == "thumb"}cm-cart-content-thumb{/if} {if $block.properties.display_delete_icons == "Y"}cm-cart-content-delete{/if}">
                                                ...
                                                {if !$product.extra.parent}
                                                    <li class="ty-cart-items__list-item clearfix">
                                                        ...
                                                        <div class="clearfix">
                                                            ...
                                                            <p>
                                                                <span class="et-product-ammount">{$product.amount}</span><span><span dir="{$language_direction}">&nbsp;x&nbsp;</span></span>{include file="common/price.tpl" value=$product.display_price span_id="price_`$key`_`$dropdown_id`" class="et-cart-product-price"}
                                                            </p>
                                                            </div>
                            ...
                            <div class="cm-cart-buttons ty-cart-content__buttons buttons-container{if $cart.amount} full-cart{else} hidden{/if}">
                                ...
                                <div class="et-cart-checkout-btn">
                                    {include file="buttons/proceed_to_checkout.tpl" but_text=__("checkout") et_cart_dropdown=true but_href="checkout.checkout?vendor_id=`$vendor_id`"|fn_url}
                                </div>
                                {/if}
                            </div>
                            {/if}
