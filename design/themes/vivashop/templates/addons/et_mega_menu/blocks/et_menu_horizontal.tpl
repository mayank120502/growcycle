	  ...
	  <ul class="ty-menu__items cm-responsive-menu">
	    {foreach from=$items item="item1" name="item1"}
	      ...
	      <li class="ty-menu__item{if !$item1.$childs} ty-menu__item-nodrop{else} cm-menu-item-responsive{/if}{if $item1.active || $item1|fn_check_is_active_menu_item:$block.type} ty-menu__item-active{/if}{if $item1.class} {$item1.class}{/if} {if $smarty.foreach.item1.first}et-menu-first-item{/if} et-main-menu-lev-1">
	        ...
	        {if isset($et_menu)}
				{if $item1.class == "no-index"}
					<a rel="nofollow" {if $item1_url} href="{$item1_url}"{/if} class="ty-menu__item-link clearfix et_menu_color_{$et_menu.et_menu_id} {if $et_icon|trim}et_menu_with_icon{/if} {if $et_menu.color.enabled|default:"N" == "Y" && $et_menu.color.color_type|default:"S"=="S"}et_use_style_color{/if}" {if $item1.new_window}target="_blank"{/if}>{strip}
