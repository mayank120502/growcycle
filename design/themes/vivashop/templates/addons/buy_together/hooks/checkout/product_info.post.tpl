{if $cart.products.$key.extra.buy_together}
  ...
  {if $smarty.capture.is_conf_prod}
    ...
    <div class="ty-product-options" id="buy_together_{$key}" title="{__("buy_together")}">
            ...
            <tr>
              <td>
                <a href="{"products.view?product_id=`$_product.product_id`"|fn_url}"
                   class="underlined"><strong>{$_product.product}</strong></a>
                  {hook name="products:product_additional_info"}
                  {/hook}
                  <br/>
                {if $_product.product_options}
                  <div class="et-buy-together_cart-option">
