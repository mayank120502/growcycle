{if $products}
	...
	{function name="et_compact_list_btns" product_id=$product_id obj_prefix=$obj_prefix}{strip}
		{if !$et_on_vs}
		  <div class="et_compact_list_btns clearfix1">
		    ...
		    {if $addons.wishlist.status == "A" && !$hide_wishlist_button}
		      {include file="addons/wishlist/views/wishlist/components/add_to_wishlist.tpl" but_id="button_wishlist_phone_`$obj_prefix``$product_id`" but_name="dispatch[wishlist.add..`$product_id`]" but_role="text" show_et_icon_grid=true}
		    {/if}
		  </div>
		{/if}
	{/strip}{/function}
