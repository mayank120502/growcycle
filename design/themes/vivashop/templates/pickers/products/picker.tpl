...
{if $view_mode != "list"}
    ...
    {if !$no_container}<div class="buttons-container picker">{/if}{if $picker_view}[{/if}
        <div class="ty-mt-m">
            {include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="products.picker?display=`$display`&picker_for=`$picker_for`&extra=`$extra_var`&checkbox_name=`$checkbox_name`&aoc=`$aoc`&data_id=`$data_id`&et_device=`$smarty.const.ET_DEVICE`" but_text=$but_text|default:__("add_products") but_role=$but_role|default:"add" but_target_id="content_`$data_id`" but_meta="ty-btn__secondary cm-dialog-opener cm-dialog-auto-size" but_rel="nofollow" but_icon="product-picker-icon ty-icon-plus"}
    {if $picker_view}]{/if}{if !$no_container}</div>{/if}
    </div>

    <div class="hidden {if $et_gift_certificate_picker}et-gift-certificate-picker{/if}" id="content_{$data_id}" title="{$but_text|default:__("add_products")}">
    </div>
