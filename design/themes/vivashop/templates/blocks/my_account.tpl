...
<div id="account_info_{$block.snapping_id}">
    ...
    <div class="ty-account-info__orders updates-wrapper track-orders" id="track_orders_block_{$block.snapping_id}">
      <form action="{""|fn_url}" method="POST" class="cm-ajax cm-post cm-ajax-full-render" name="track_order_quick">
        ...
        <div class="ty-account-info__orders-input ty-control-group ty-input-append">
          ...
          {include file="buttons/go.tpl" but_name="orders.track_request" alt=__("go") et_icon="ty-icon-search"}
  ...
  <div class="ty-account-info__buttons buttons-container">
    ...
    {else}
            <a href="{if $runtime.controller == "auth" && $runtime.mode == "login_form"}{$config.current_url|fn_url}{else}{"auth.login_form?return_url=`$return_current_url`"|fn_url}{/if}" data-ca-target-id="login_block{$block.snapping_id}" class="cm-dialog-opener cm-dialog-auto-size ty-btn ty-btn__primary" rel="nofollow">{__("sign_in")}</a><a href="{"profiles.add"|fn_url}" rel="nofollow" class="ty-btn ty-btn__secondary">{__("register")}</a>
