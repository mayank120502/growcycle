<div class="ty-sort-container">
	...
	{if $pagination.total_items}
		...
		<a id="sw_elm_pagination_steps" class="ty-sort-dropdown__wrapper cm-combination">{$pagination.items_per_page} <span class="visible-desktop">{__("per_page")}</span>{include_ext file="common/icon.tpl" class="ty-icon-down-micro ty-sort-dropdown__icon"}</a>
			<ul id="elm_pagination_steps" class="ty-sort-dropdown__content cm-popup-box hidden">
				{foreach from=$product_steps item="step"}
					{if $step != $pagination.items_per_page}
						<li class="ty-sort-dropdown__content-item">
							<a class="{$ajax_class} ty-sort-dropdown__content-item-a" href="{"`$range_url`&items_per_page=`$step`"|fn_url}" data-ca-target-id="{$pagination_id}" rel="nofollow">{$step} <span class="visible-desktop">{__("per_page")}</span></a>
						</li>
					{/if}
				{/foreach}
			</ul>
