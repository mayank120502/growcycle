...
<thead>
		...
		<th class="ty-cart-content__title et-unit-price ty-right">{__("unit_price")}</th>
...
{if !$cart.products.$key.extra.parent}
			...
			<div class="ty-cart-content__image cm-reload-{$obj_id}" id="product_image_update_{$obj_id}">
				{hook name="checkout:product_icon"}
					<a href="{"products.view?product_id=`$product.product_id`"|fn_url}">
						...
						{/if}
					{include file="common/image.tpl" obj_id=$key images=$product.main_pair image_width=$image_width image_height=$image_height}</a>
				{/hook}
    ...
    {hook name="products:product_additional_info"}
		<div class="ty-cart-content__sku ty-sku cm-hidden-wrapper{if !$product.product_code} hidden{/if} " id="sku_{$key}">
			<strong>{__("sku")}:</strong> <span class="cm-reload-{$obj_id}" id="product_code_update_{$obj_id}">{$product.product_code}<!--product_code_update_{$obj_id}--></span>
		</div>
	...
	<td class="ty-cart-content__product-elem ty-cart-content__price et-unit-price cm-reload-{$obj_id}" id="price_display_update_{$obj_id}">
...
<!--price_display_update_{$obj_id}--></td>
	...
	<td class="ty-cart-content__product-elem ty-cart-content__qty {if $product.is_edp == "Y" || $product.exclude_from_calculate} quantity-disabled{/if}">
		...
		<div class="quantity cm-reload-{$obj_id}{if $settings.Appearance.quantity_changer == "Y"} changer et-small-value-changer{/if}" id="quantity_update_{$obj_id}">
	...
	<td class="ty-cart-content__product-elem ty-cart-content__price cm-reload-{$obj_id} et-total-price" id="price_subtotal_update_{$obj_id}">
