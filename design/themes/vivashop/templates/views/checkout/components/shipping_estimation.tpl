        ...
        <div id="{$shipping_estimation_block_id}">
            ...
            <form class="cm-ajax" name="{$prefix}estimation_form{$id_suffix}" action="{""|fn_url}" method="post">
                ...
                <div class="ty-control-group">
                    ...
                    <input 
                        type="text" 
                        class="cm-state cm-location-estimation{$class_suffix} ty-input-text-medium {if $states[$_country]}hidden{/if}" 
                        id="{$prefix}elm_state{$id_suffix}{if $states[$_country]}_d{/if}" 
                        name="customer_location[state]" 
                        size="20" 
                        maxlength="64" 
                        value="{$_state}" 
                        {if $states[$_country]}disabled="disabled"{/if} 
                    />
                </div>
...
{if $group.shipping_by_marketplace}
                            {continue}
                        {/if}
                        <div class="et-shipping-popup_option-wrapper">
                            {* Group name *}
                            ...
                            {if $group.shippings && !$group.all_edp_free_shipping && !$group.shipping_no_required}
                                {foreach from=$group.shippings item="shipping" name="estimate_group_shipping"}
                                    {if !$show_only_first_shipping || $smarty.foreach.estimate_group_shipping.first}
                                        ...
                                        {if $shipping.rate}
                                            {capture assign="rate"}{include file="common/price.tpl" value=$shipping.rate class="ty-strong"}{/capture}
                                            {if $shipping.inc_tax}
                                                {assign var="rate" value="`$rate` ("}
                                                {if $shipping.taxed_price && $shipping.taxed_price != $shipping.rate}
                                                    {capture assign="tax"}{include file="common/price.tpl" value=$shipping.taxed_price class="ty-nowrap ty-strong"}{/capture}
                            ...
                            {else}
                                ...
                                {else}
                                <p>{__("text_no_shipping_methods") nofilter}</p>
                                {/if}
                            {/if}
                        </div>
                    {/foreach}
                    ...
                    <p><strong class="et-shipping_estimation_total">{__("total")}:</strong>&nbsp;{include file="common/price.tpl" value=$cart.display_shipping_cost class="ty-price ty-strong et-price"}</p>
...
<div class="ty-estimation-buttons buttons-container" id="shipping_estimation_buttons">
    {if $runtime.mode == "shipping_estimation" || $smarty.request.show_shippings == "Y"}
        {include file="buttons/button.tpl" but_text=__("recalculate_rates") but_external_click_id="but_get_rates_{$location}" but_role="text" but_meta="ty-btn__primary cm-external-click ty-float-right ty-estimation-buttons__rate"}

        {include file="buttons/button.tpl" but_text=__("select_shipping_method") but_external_click_id="but_select_shipping" but_meta="ty-btn__primary cm-external-click cm-dialog-closer"}
    {else}
        {include file="buttons/button.tpl" but_text=__("get_rates") but_external_click_id="but_get_rates_{$location}" but_meta="ty-btn__primary cm-external-click"}
    {/if}
