...
{if $id && $page_type != $smarty.const.PAGE_TYPE_LINK && (!"ULTIMATE"|fn_allowed_for || $runtime.company_id)}
    {$view_uri = "pages.view?page_id=`$id`"|fn_get_preview_url:$page_data:$auth.user_id:$default_language}
{/if}
...
{capture name="buttons"}
    {if $id}
        {capture name="tools_list"}
            {if $is_allow_update_pages}
                {foreach $page_types as $_k => $_p}
