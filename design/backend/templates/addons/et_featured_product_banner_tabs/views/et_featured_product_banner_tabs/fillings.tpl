<div>
	<div class="control-group">
	  ...
	  <div class="controls">
	    <select 
	      id="filling_select" 
	      name="block_data[tab][{$id}][content][items][filling]" 
	      onchange="fn_et_fillings(this.value,{$id});">
  ...
  {if $filling == 'manually'}
    <div class="control-group cm-no-hide-input">
      ...
      {include file="views/products/components/picker/picker.tpl"
          input_name="block_data[tab][{$id}][content][items][item_ids]"
          item_ids=$prod_ids
          multiple=true
          view_mode="external"
  ...
  {if $fillings.$filling.settings|is_array}
    {foreach from=$fillings.$filling.settings item=setting_data key=setting_name}
      {include file="views/block_manager/components/setting_element.tpl" 
        ...
        html_name="block_data[tab][{$id}][content][items][`$setting_name`]"
        editable=true}
    {/foreach}
  {/if}
