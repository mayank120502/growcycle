...
{$type_time_val = $date_val|fn_cp_check_time_format}
...
<div class="calendar">
    ...
    {if $show_time}
        <input class="input-time" data-ca-meta-class="{$meta_class}" size="5" maxlength="5" type="text" name="{$time_name}" value="{if $date_val}{$date_val|fn_parse_date|date_format:"%I:%M"}{/if}" placeholder="00:00" />
        <select class="cp_type_time" name="{$type_time_name}" >
            <option value="A" {if $type_time_val == "A"}selected="selected"{/if}>{__("cp_discussion.am")}</option>
            <option value="P" {if $type_time_val == "P"}selected="selected"{/if}>{__("cp_discussion.pm")}</option>
        </select>
    {/if}
