<div class="hidden" id="content_cp_sms_send">
    <div class="control-group">
        ...
        <div class="controls">
            <input type="hidden" name="update_order[cp_sms_receive]" value="N">
            <input {if $order_info.cp_sms_receive == "Y"}checked="checked"{/if} id="cp_sms_receive" type="checkbox" name="update_order[cp_sms_receive]" value="Y">
        </div>
    </div>
    {if $cp_sms_show_send_tab}
        {include file="addons/cp_sms_notifications/components/send_content.tpl" data=$order_info}
    {/if}
