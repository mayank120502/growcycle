...
{capture name="cp_img_main"}
    ...
    {else}
        ...
        {/if}
        <img class="ty-pict {$valign} {$class} {if $lazy_load && !$no_lazyOwl}lazyOwl{/if} {if $generate_image}ty-spinner{/if} cm-image {$smarty.capture.cp_img_class}" {if $obj_id && !$no_ids}id="det_img_{$obj_id}"{/if} {if $generate_image}data-ca-image-path="{$image_data.image_path}"{/if} {hook name="cp_image:img_src"}{if $lazy_load}data-{/if}src="{if $generate_image}{$images_dir}/icons/spacer.gif{else}{$image_data.image_path}{/if}"{/hook} {if $image_onclick}onclick="{$image_onclick}"{/if} {$image_attributes|render_tag_attrs nofilter} {$smarty.capture.cp_img_extra nofilter} />
    {/if}
{/capture}
