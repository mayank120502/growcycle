...
{if $multiple}
    ...
    {if !$hide_delete_button && !$view_only}
        <a onclick="Tygh.$.cePicker('delete_js_item', '{$holder}', '{$category_id}', 'c'); return false;" class="ty-delete-big" title="{__("remove")}">{include_ext file="common/icon.tpl" class="ty-icon-cancel-circle ty-delete-big__icon"}</a>
    {/if}
{else}
    <input class="ty-input-text cm-picker-value-description" type="text" value="{$category}" {if $display_input_id}id="{$display_input_id}"{/if} name="category_name" readonly="readonly" {$extra} />
{/if}
