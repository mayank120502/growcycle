...
{if !$config.tweaks.dev_js}
    {script src="js/lib/jquery/jquery-{$jquery_version}.min.js" no-defer=true}
    {script src="js/lib/jquery/jquery-migrate-{$jquery_migrate_version}.min.js" no-defer=true}
{/if}
...
{script src="design/themes/vivashop/js/et.js"}
{script src="design/themes/vivashop/js/bootstrap-select.js"}

{script src="js/tygh/editors/`$settings.Appearance.default_wysiwyg_editor`.editor.js"}
{script src="design/themes/vivashop/js/et-responsive.js"}
...
{if $smarty.const.ET_DEVICE == "T"}
<script>
(function(_, $) {

    var et_ft_width = $.cookie.get('et_ft_width');
    if (!et_ft_width){
        $.cookie.set('et_ft_width', window.innerWidth);
        location.reload();
    }
    if (et_ft_width<=1024 && window.innerWidth>1024){
        $.cookie.set('et_ft_width', window.innerWidth);
        location.reload();
    }else if (et_ft_width!=window.innerWidth && window.innerWidth<1024){
        $.cookie.set('et_ft_width', window.innerWidth);
        location.reload();
