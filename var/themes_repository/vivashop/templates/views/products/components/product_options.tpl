...
<div id="option_{$id}_AOC">
  <div class="cm-picker-product-options ty-product-options" id="opt_{$obj_prefix}{$id}">
    {foreach $product_options as $po}
      ...
      <div class="ty-control-group ty-product-options__item {if !$capture_options_vs_qty}product-list-field{/if} clearfix et_option_type_{$po.option_type}"
      ...
      >
        {if !($po.option_type && (
          ...
          <label id="option_description_{$id}_{$po.option_id}"
               ...
               {/if}
          >
            {$po.option_name}{if $po.description|trim}{include file="common/tooltip.tpl" tooltip=$po.description}{/if}:
          </label>
              ...
              <bdi>
                <select name="{$name}[{$id}][product_options][{$po.option_id}]"
                  ...
                  {if $product.exclude_from_calculate && !$product.aoc || $po.disabled || $disabled}
                    disabled="disabled"
                  {/if}
                  class="selectpicker {if $product.exclude_from_calculate && !$product.aoc || $po.disabled || $disabled}disabled{/if}"
                >
                    ...
                    {if !($po.disabled || $disabled) || (($po.disabled || $disabled) && $po.value && $po.value == $vr.variant_id)}
                      {capture name="modifier"}
                        {include file="common/modifier.tpl"
                            ...
                            display_sign=true
                            et_normal_price=true
                        }
                      {/capture}
                      {capture name="img"}
                        {if $vr.image_pair.image_id}
                          {if $vr.variant_id == $selected_variant}{assign var="_class" value="product-variant-image-selected"}{else}{assign var="_class" value="product-variant-image-unselected"}{/if}
                          {include file="common/image.tpl" class="$_class" images=$vr.image_pair image_width="35" image_height="35"}
                        {/if}
                      {/capture}
              
                      {capture name="et_select"}{strip}
                        {$smarty.capture.img}<span><strong>{$vr.variant_name}</strong>&nbsp;<span>{if $show_modifiers}{hook name="products:options_modifiers"}{if $vr.modifier|floatval}({$smarty.capture.modifier|strip_tags|strip:'' nofilter}){/if}{/hook}{/if}</span></span>
                      {/strip}{/capture}
                      ...
                      <option value="{$vr.variant_id}"
                          {if $po.value == $vr.variant_id}
                            ...
                            selected="selected"
                          {/if} 
                          data-content="{$smarty.capture.et_select|trim nofilter}"
                      >{strip}
                        {$vr.variant_name}
                        {if $show_modifiers}
        ...
        {elseif $po.option_type == "ProductOptionTypes::RADIO_GROUP"|enum} {*Radiobutton*}
          {if $po.variants}
            ...
            <ul id="option_{$obj_prefix}{$id}_{$po.option_id}_group" class="ty-product-options__elem">
              {if !$po.disabled && !$disabled}
                {foreach $po.variants as $vr}{strip}
                  <li class="et_radio">
                    ...
                    >
                      <input  type="radio"
                              ...
                              {/if}
                              {if $product.options_update}
                                onclick="fn_change_options('{$obj_prefix}{$id}', '{$id}', '{$po.option_id}');Tygh.$('#option_{$obj_prefix}{$id}_{$po.option_id}_group .et_radio_text.selected').removeClass('selected'); Tygh.$('#option_description_{$id}_{$po.option_id}_{$vr.variant_id} .et_radio_text').addClass('selected');"
                              {else}
                                onclick="fn_change_variant_image('{$obj_prefix}{$id}', '{$po.option_id}', '{$vr.variant_id}'); Tygh.$('#option_{$obj_prefix}{$id}_{$po.option_id}_group .et_radio_text.selected').removeClass('selected'); Tygh.$('#option_description_{$id}_{$po.option_id}_{$vr.variant_id} .et_radio_text').addClass('selected');"
                              {/if}
                              {if $product.exclude_from_calculate && !$product.aoc || $po.disabled || $disabled}
                                disabled="disabled"
                              {/if}
                      />
                      <span class="et_radio_text {if $po.value == $vr.variant_id }selected{/if}"><strong>{$vr.variant_name}</strong>&nbsp;
                        {if $show_modifiers}
                          {hook name="products:options_modifiers"}
                            {if $vr.modifier|floatval}
                              ({include file="common/modifier.tpl"
