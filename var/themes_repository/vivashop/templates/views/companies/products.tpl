...
<div id="products_search_{$block.block_id}">
	{if $products}
		...
		{if $layouts.$selected_layout.template}
			{include file="`$layouts.$selected_layout.template`" columns=$product_columns show_qty=true use_vendor_url=true}
		{/if}
...
{capture name="mainbox_title"}<span class="et-block-title-border">{$_title}</span><span class="ty-mainbox-title__right" id="products_search_total_found_{$block.block_id}">{$title_extra nofilter}<!--products_search_total_found_{$block.block_id}--></span>{/capture}
{/hook}