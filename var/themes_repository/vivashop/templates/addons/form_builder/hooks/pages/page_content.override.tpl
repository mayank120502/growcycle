	...
	{else}
		...
		<div class="ty-form-builder">
			<form action="{""|fn_url}" method="post" name="forms_form" enctype="multipart/form-data">
				...
				<div class="et-columns-wrapper wrap">
					{foreach from=$page.form.elements key="element_id" item="element" name="form_lements"}
						{if $element.element_type == $smarty.const.FORM_SEPARATOR}
							<hr class="ty-form-builder__separator et-column1" />
						{elseif $element.element_type == $smarty.const.FORM_HEADER}
							{include file="common/subheader.tpl" title=$element.description class="ty-subheader et-column1"}
	    
						{elseif $element.element_type != $smarty.const.FORM_IP_ADDRESS && $element.element_type != $smarty.const.FORM_REFERER}
							<div class="ty-control-group {if $element.element_type=="T" || $element.element_type == $smarty.const.FORM_RADIO}et-column1{else}et-column2{/if}">
								...
								{elseif $element.element_type == $smarty.const.FORM_EMAIL || $element.element_type == $smarty.const.FORM_NUMBER || $element.element_type == $smarty.const.FORM_PHONE}
									...
									{/if}
	                <input id="elm_{$element.element_id}" class="ty-input-text {if $element.element_type == $smarty.const.FORM_PHONE} cm-mask-phone{/if}" size="50" type="text" name="form_values[{$element.element_id}]" value="{$form_values.$element_id}" />
		    ...
		    {/hook}
				...
				<div class="ty-form-builder__buttons buttons-container">
					{include file="buttons/button.tpl" but_role="submit" but_text=__("submit") but_meta="ty-btn__primary" but_name="dispatch[pages.send_form]"}
				</div>
			</form>
		</div>
