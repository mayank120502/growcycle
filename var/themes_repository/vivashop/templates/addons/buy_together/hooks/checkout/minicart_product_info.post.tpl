    ...
    <ul class="ty-buy-together-cart-items__list">
        {foreach from=$_cart_products item="_product" key="_key"}
            {if $_product.extra.parent.buy_together == $key}
                <li class="ty-buy-together-cart-items__list-item clearfix">
                    {if $block.properties.products_links_type == "thumb"}
                        <div class="ty-cart-items__list-item-image">
                            {include file="common/image.tpl" image_width="50" image_height="50" images=$_product.main_pair no_ids=true}
                        </div>
                    {/if}
                    <div class="ty-cart-items__list-item-desc">
                        ...
                        <p>
                            <span class="et-product-ammount">{$_product.amount}</span><span>&nbsp;x&nbsp;</span>{include file="common/price.tpl" value=$_product.display_price span_id="price_`$key`_`$dropdown_id`" class="et-cart-product-price"}</p>
                    </div>
                </li>
