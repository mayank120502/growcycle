...
<tr>
    ...
    <td class="ty-cart-content__product-elem ty-cart-content__description" style="width: 50%;">
        {if !$gift.extra.exclude_from_calculate}
            ...
            {if !$gift.extra.exclude_from_calculate}
                <a class="{$ajax_class} cm-post ty-delete-big" href="{"gift_certificates.delete?gift_cert_id=`$gift_key`&redirect_url=`$c_url`"|fn_url}"  data-ca-target-id="cart_items,checkout_totals,cart_status*,checkout_steps,checkout_cart" title="{__("remove")}">
                    {include_ext file="common/icon.tpl"
                        class="ty-icon-cancel-circle ty-delete-big__icon"
                    }
                </a>
            {/if}
                ...
                <div class="ty-cart-content-products__item">
                    <div>
                        ...
                        {if $use_ajax == true}
                            {assign var="ajax_class" value="cm-ajax"}
                        {/if}
                        <a class="{$ajax_class} ty-delete-big" href="{"checkout.delete?cart_id=`$key`&redirect_url=`$c_url`"|fn_url}" data-ca-target-id="cart_items,checkout_totals,cart_status*,checkout_steps" title="{__("remove")}">{include_ext file="common/icon.tpl" class="ty-icon-cancel-circle ty-delete-big__icon"}</a>
