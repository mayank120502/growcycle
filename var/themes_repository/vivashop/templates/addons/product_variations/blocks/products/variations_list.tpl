    ...
    <table class="ty-variations-list ty-table ty-table--sorter" data-ca-sortable="true" data-ca-sort-list="[[1, 0]]">
        <thead>
            <tr>
                ...
                {if $show_product_amount}
                    <th class="ty-variations-list__title ty-right cm-tablesorter" data-ca-sortable-column="true" width="15%">{__("availability")}</th>
                {/if}
        ...
        <tbody>
            {foreach $products as $key => $product name="variations_list"}
                {if $use_vendor_url}
                    {$prod_url_dispatch="companies.product_view"}
                    {$et_prod_url_dispatch_extra="&company_id=`$smarty.request.company_id`"}
                {else}
                    {$prod_url_dispatch="products.view"}
                    {$et_prod_url_dispatch_extra=""}
                {/if}

                {$variation_link="`$prod_url_dispatch`?product_id=`$product.product_id``$et_prod_url_dispatch_extra`"|fn_url}

                {$obj_id=$product.product_id}
                {$obj_id_prefix="`$obj_prefix``$product.product_id`"}

                {include file="common/product_data.tpl" product=$product et_show_icon_atc=true show_select_variations_button=false et_variation_list=true}
