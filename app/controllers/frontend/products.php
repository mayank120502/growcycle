...
//
    ...
    if(!empty($product['company_id'])){
        $userID = db_get_field('SELECT * FROM ?:users WHERE company_id = ?s', $product['company_id']);
        $userData = fn_get_user_info($userID);
    }
    ...
    Tygh::$app['view']->assign('user_data', $userData ?? null);
