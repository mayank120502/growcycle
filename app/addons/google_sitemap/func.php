 ...
 * @return string[]
 */
function fn_google_sitemap_generate_link($type, $id, $languages, $extra = [], $storefront_id = null, $companyId = 0)
{
    switch ($type) {
        case 'product':
     ...
     */
    fn_set_hook('google_sitemap_generate_link_post', $type, $id, $languages, $extra, $storefront_id, $links, $companyId);

    //Code for Arsen
    if ($type == 'product' && $companyId != '0') {
        $link = 'companies.product_view&product_id='.$id.'&company_id='.$companyId;
        $links['main_link'] = fn_url($link, SiteArea::STOREFRONT, fn_get_storefront_protocol(), key($languages));
    }
...
)
{
    ...
    while ($params['pid'] = db_get_fields('SELECT product_id FROM ?:products WHERE status = ?s ORDER BY product_id ASC ?p', ObjectStatuses::ACTIVE, db_paginate($page, $products_per_page))) {
        ...
        foreach ($products as $product) {
            ...
            $links = fn_google_sitemap_generate_link('product', $product['product_id'], $languages, [], $storefront->storefront_id, $product['company_id']);
            ...
            /*Artur for not duplications*/
// if(isset($exclude_duble[ md5( $item ). '_'. md5($links) ]) ) continue;
// $exclude_duble[ md5( $item ). '_'. md5($links) ] = 1;

            if (isset($exclude_duble[md5($item)])) continue;
            $exclude_duble[md5($item)] = 1;
