	...
	$.fn.extend({ 
		tag: function() { 
			...
			this.each(function(){
				...
				var $this = $(this);
				if ($this.data('caName')){				
					attr.inputName = $this.data('caName');
				}
				
				if ($this.data('caKeys')){					
					attr.key = ($this.data('caKeys') + '').split(",");							
				}	
				...
				//Add textbox
				$('<input type="text" value="" name="add_tag" placeholder="" class="new_tag_input" onkeydown="if(event.keyCode == 13) { return false; }" />')
				...
				/*if ($this.data('caSynonyms')){				
					($this.data('caSynonyms')+'').split(",").forEach(function (item) {						
						$this.addTag(item);	
					});			
		...
		updateTagsArray: function() {
			...
			$this.children('.tags_array').val(('array()' + JSON.stringify(tags)));
...
$(document).on('click', '.cls-synonyms .tag', function(){	
	//Remove tag
	$(this).animate({
		opacity: 0
	}, 150, function() {
