    ...
    if ($elems.length) {
      $elems.each(function () {
        ...
        if ($target_elem.length) {
          $(this).appendTo('body');
          $target_elem.data('ceTooltipPosition', 'center top').data('ceTooltipClass', 'ty-gdpr-tooltip ty-gdpr-tooltip--light').ceTooltip({
            ...
            onHide: function onHide() {
              var $tip = this.getTip();
              if (typeof $tip !== 'undefined' && $tip.hasClass('ty-gdpr-tooltip-popup')) {
                $tip.addClass('cm-gdpr-tooltip_hidden');
              }
            }
          ...
          $('#gdpr_tooltip_' + target_elem_id).find('.cm-gdpr-tooltip--close').on('touchstart', function () {
            ...
            $('#gdpr_tooltip_' + target_elem_id).addClass('cm-gdpr-tooltip_hidden');
          });
        }
      });
    }
